<?php
/*
 * Plugin Name: ColorPress
 * Version: 0.1
 * Plugin URI: http://www.codenamecuttlefish.com/project-files/ColorPress-colourlovers-wordpress-widget/
 * Description: A customizable wordpress widget that displays top or new COLOURLovers palettes or patterns on your blog.
 * Author: serostar
 * Author URI: http://www.serostar.com/
 */

//check if the the stylesheet exists, then enqueue it into the header
$myStyleUrl = WP_PLUGIN_URL . '/ColorPress/styles/ColorPressStyle.css';
$myStyleFile = WP_PLUGIN_DIR . '/ColorPress/styles/ColorPressStyle.css';
if ( file_exists($myStyleFile) ) {
    wp_register_style('ColorPressStylesheet', $myStyleUrl);
    wp_enqueue_style( 'ColorPressStylesheet');
}


 
class ColorPressWidget extends WP_Widget
{

	function ColorPressWidget(){
		$widget_ops = array('classname' => 'ColorPress_widget', 'description' => __( "Display your COLOURLovers.com palettes and patterns") );
		$this->WP_Widget('clwidget', __('ColorPress'), $widget_ops);
	}
...
<div class="cl_results">
	...
	<?php	
		echo '<div style="text-align:left;"><small>' . ucwords($type) . 's from <a href="http://www.colourlovers.com" target="_blank">COLOURLovers.com</a></small></div><br />';
		echo '</div>';
	...
	*/
	function ColorPressWidgetInit() {
	register_widget('ColorPressWidget');
	}	
	add_action('widgets_init', 'ColorPressWidgetInit');
?>